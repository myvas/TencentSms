...
namespace AspNetCore.QcloudSms
{
    public class QcloudSmsManager : ISmsSender
    {
        ...
        /// <param name="content">腾讯云规定：短信内容必须匹配事先提交并通过审核的模版。例如：【短信签名】{1}为您的验证码。如非本人操作，请忽略本短信。</param>
        /// <returns></returns>
        ...
        public async Task<bool> SendSmsWithBrandNameAsync(string mobile, string content)
        {
            return await SendSmsAsync(mobile, $"【{_options.BrandName}】{content}");
        }
    }
}
